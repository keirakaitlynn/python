...
def get_interstate_number():
    ...
    while (interstate_number <= 0):
        interstate_str = (input("Please enter a US Interstate Highway route number: "))
        # If input begins w/ "Interstate ", sets interstate # to anything after it
        if len(interstate_str) >= 10:
            if interstate_str[0:10] == "Interstate":
                interstate_number = int(interstate_str[11:])
        # If input begins w/ "I-", sets interstate # to anything after it
        elif interstate_str.startswith("I") == True and interstate_str[1] == "-":
            interstate_number = int(interstate_str[2:5])
        # If input begins w/ "I", sets interstate # to anything after it
        elif interstate_str[0] == "I":
            interstate_number = int(interstate_str[1:3])
        # Otherwise, returns the integer of input
        else:
            interstate_number = int(interstate_str)
